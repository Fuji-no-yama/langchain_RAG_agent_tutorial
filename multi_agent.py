...
langchain.debug = (
    True  # コマンドライン上でLLMとのやり取りを可視化できる。必要なければFalseに
)
...
def format_file(
    ...
    str
):  # zip形式のファイルを解凍してremove_rubyに入れる関数(zipの中身のファイル名を返す)
...
def init_RAG_tool(
    filename: str
) -> langchain_core.tools.Tool:  # 与えられたファイルに関するRAGツールを作成する関数
    # ツール概要作成用のchain
    if (
        st.session_state.RAG_sourcefiles[filename] == ""
    ):  # 概要が登録されていなければchainで作成し登録
        prompt = ChatPromptTemplate.from_template(
            "以下の文章を読んでメインタイトルをつけて, 4個程度のキーワードを挙げてください。\n #文章:\n{sentence}"
        )
        model = ChatOpenAI(
    ...
    vectorstore = FAISS.load_local(
        "/workspace/DBs/vector_" + (filename.rsplit(".", 1)[0]),
        embedding,
...
def create_agent_chain() -> AgentExecutor:  # エージェントを作る関数
    chat = ChatOpenAI(
        model=os.environ["OPENAI_API_MODEL"],
        temperature=float(os.environ["OPENAI_API_TEMPERATURE"]),
        streaming=True,
    ...
    tools = tools + [
        init_RAG_tool(x) for x in list(st.session_state.RAG_sourcefiles.keys())
    ]
...
if (
    "RAG_sourcefiles" not in st.session_state
):  # RAGの参照ファイルとその概要(RAGtool作成に必要)の辞書型オブジェクト(docの中身で初期化)
    files = [
        f for f in os.listdir("/workspace/doc") if not f.startswith(".")
    ]  # 隠しファイルを除くファイル一覧を取得
    st.session_state.RAG_sourcefiles = {file: "" for file in files}

uploaded_file = st.file_uploader(
    "青空文庫のZIPファイルを選択してください", type=["zip"]
)
if (
    uploaded_file is not None
):  # アップロードされたファイルがある場合は適宜処理してソースファイルとして追加する
    ...
    st.session_state.agent = (
        create_agent_chain()
    )  # ファイルが追加された場合agentも作成し直す

df = pd.DataFrame(
    {  # 使用可能ツールのテーブル表示
        "使用可能agentツール": ["duck duck go(web検索)"]
        + list(st.session_state.RAG_sourcefiles.keys()),
    }
)
st.table(df)
...
with st.spinner("Vector DBの作成中..."):
    for filename in list(
        st.session_state.RAG_sourcefiles.keys()
    ):  # vectorDBのディレクトリがない場合に作成する(初回起動時のみ)
        print(filename + "の作成中")
...
if "chain_memory" not in st.session_state:  # chain用memoryを初期化(上限2個まで)
    st.session_state.chain_memory = ConversationBufferWindowMemory(
        k=2, memory_key="history", return_messages=True
    )
...
if prompt:
    ...
    with st.chat_message("assistant"):  # 応答の生成と表示
        agent_result = st.session_state.agent.invoke(
            {
                "question": prompt,
                "history": st.session_state.chain_memory.load_memory_variables({})[
                    "history"
                ],
            }
        )
    ...
    st.session_state.chain_memory.save_context(
